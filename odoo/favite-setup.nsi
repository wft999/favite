...
!define SERVICENAME "FaviteTools"
!define PRODUCT_NAME "FaviteTools"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Favite Tools ${VERSION}"
...
OutFile "favite-tools-setup-${VERSION}.exe"
...
InstallDir "$PROGRAMFILES\FaviteTools ${VERSION}"
...
!define POSTGRESQL_EXE_FILENAME "postgresql-9.5.13-1-windows.exe"
!define POSTGRESQL_EXE "${STATIC_PATH}\${POSTGRESQL_EXE_FILENAME}"

!define MUI_ABORTWARNING
!define MUI_ICON "${PIXMAPS_PATH}\openerp-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\favite-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\favite-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\favite-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "http://www.favite.com/service-3.asp"
...
; English
LangString DESC_OpenERP_Server ${LANG_ENGLISH} "Install the FaviteTools Server with all the standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Favite."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Favite for Partnership and/or Support"
...
LangString TITLE_OpenERP_Server ${LANG_ENGLISH} "FaviteTools Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start FaviteTools"
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@favite.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "http://www.favite.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@favite.com"
